...
module.exports = async (client, alivePlayersBefore) => {
  ...
  const serialKillers = alivePlayersBefore.filter(p => db.get(`player_${p}`).role === "Serial Killer") // get the alive Serial Killers array - Array<Snowflake>
  
  // loop through each serial killer
  for (let sk of serialKillers) {
    
    let attacker = db.get(`player_${sk}`) // the attacker object - Object
    
    // check if the sk has a target
    if (attacker.target) {
      
      // delete the target
      db.delete(`player_${sk}.target`) // don't worry, this won't affect the current target
      ...
      if (guy.status === "Alive") {
        ...
        if (typeof result === "object") {
          ...
          if (typeof attacker.hypnotize === "string") {
            await dayChat.send(`${getEmoji("serial_killer_knife", client)} The Dreamcatcher compelled the Serial Killer to stab **${players.indexOf(result.id)+1} ${result.username} (${getEmoji(result.role?.toLowerCase()?.replace(/\s/g, "_"), client)} ${result.role})**!`)
          } else {
            await dayChat.send(`${getEmoji("serial_killer_knife", client)} The Serial Killer stabbed **${players.indexOf(result.id)+1} ${result.username} (${getEmoji(result.role?.toLowerCase()?.replace(/\s/g, "_"), client)} ${result.role})**!`)
          }
